...
export class TurboMinion {
  ...
  connect() {
    const serverUrl = this.client.getServer(this.team);
  ...
  handleBuffer(buffer) {
    ...
    switch (messageType) {
      ...
      case 241:
        ...
        const serverMatch = serverUrl.match(
      ...
      case 255:
        this.handleMessage(
          helper.uncompressBuffer(reader.toBuffer().subarray(5), Buffer.alloc(reader.readUInt32LE()))
        );
        break;
    }
  }
  ...
  useMassBoost() {
    ...
    if (config.facebookBotSettings.useMassBoost && this.facebookBots) {
      manager.buyMassBoost(this.t, (buyBuffer) => { this.send(buyBuffer, true); });
      manager.setMassBoostExpire(this.t, (expireBuffer) => { this.send(expireBuffer, true); });
  ...
  handleMessage(buffer) {
    ...
    const messageType = reader.readUInt8();
    switch (messageType) {
      case 16: this.updateNodes(reader); break;
      case 64: this.updateOffset(reader); break;
    }
  }
  ...
  updateNodes(reader) {
    ...
    while (true) {
      ...
      let skinName = null, color = null, name = null, extraFlags = 0;
      if (flags & 128) extraFlags = reader.readUInt8();
      if (flags & 2) color = helper.intToHex((reader.readUInt8() << 16) | (reader.readUInt8() << 8) | reader.readUInt8());
      ...
      let accountId = 0;
      if (extraFlags & 4) { reader.readOffset += 4; accountId = reader.readUInt32LE(reader.readOffset - 4); }
      let cell = this.playerCells[cellId];
      if (!cell) { cell = new Entity(cellId, accountId); this.playerCells[cellId] = cell; }
      ...
      if (this.myCellIds[cellId] && this.ownCells.indexOf(cell) === -1) { cell.isMine = true; this.ownCells.push(cell); }
      cell.x = x; cell.y = y; cell.size = size;
      cell.isFood = isFood; cell.isVirus = isVirus;
      cell.agitated = isAgitated; cell.isFriend = isFriend; cell.accountID = accountId;
    }
    ...
    if (this.isAlive && this.ownCells.length === 0) {
      ...
      if (skinSettings.enable && this.facebookBots) {
        ...
        manager.changeSkin(this.t, skinName, (skinBuffer) => { this.send(skinBuffer, true); });
      }
  ...
  updateOffset(reader) {
    const minX = reader.readDoubleLE(), minY = reader.readDoubleLE();
    const maxX = reader.readDoubleLE(), maxY = reader.readDoubleLE();
  ...
  move() {
    ...
    for (const { x, y, size } of cells) { cx += x; cy += y; csize += size; }
    cx /= cells.length;
    cy /= cells.length;

    const clientX = this.client.userX / this.rX + this.offsetX;
    const clientY = this.client.userY / this.rY + this.offsetY;
    this.isNearMouse = Math.hypot(clientX - cx, clientY - cy) < 4000 + helper.size2mass(csize) * 0.5;

    let targetX = clientX;
    let targetY = clientY;

    const useAI = (this.team === 0 ? this.client.botAiA : this.client.botAiB);
    const useVShield = (this.team === 0 ? this.client.botVShieldA : this.client.botVShieldB);
    const cMass = helper.size2mass(csize);
    const massOver2000 = cMass > 2000;
    const botName = this.client.getBotName(this.team);
    const botNameEncoded = unescape(encodeURIComponent(botName));
    ...
    for (const cell of Object.values(this.playerCells)) {
      ...
      if (distance < 150) enemies.push({ dx, dy, distance, sizeRatio: helper.size2mass(cell.size) / cMass });
    }

    if (enemies.length > 0) {
      const dxM = clientX - cx, dyM = clientY - cy, md = 1 + Math.hypot(dxM, dyM);
