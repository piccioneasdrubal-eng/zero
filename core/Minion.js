...
export class Minion {
  ...
  onclose() {
    logger.info(`Minion onclose: isAlive=${this.isAlive}, hadToken=${!!this.token}`);
    this.isClosed = true;
  ...
  onerror(e) {
    logger.warn(`Minion onerror: ${e?.message || "unknown"}`);
    this.isClosed = true;
    this.clearTimeouts();
    this.clearIntervals();
  ...
  onmessage({ data: buffer }) {
    ...
    if (this.decryptionKey) {
      processedBuffer = helper.xorBuffer(processedBuffer, this.decryptionKey ^ 31128);
    }
    ...
    switch (messageType) {
      ...
      case 241:
        ...
        if (serverMatch && serverMatch[1]) {
          this.encryptionKey = helper.murmur2("" + serverMatch[1] + reader.readStringNT("utf-8"), 255);
        }
        break;
      case 242:
  ...
  updateNodes(reader) {
    ...
    while (true) {
      ...
      let name = null;
      let extraFlags = 0;
      if (flags & 128) extraFlags = reader.readUInt8();
      if (flags & 2) {
        color = helper.intToHex((reader.readUInt8() << 16) | (reader.readUInt8() << 8) | reader.readUInt8());
      }
      ...
      if (skinName !== null) cell.name = unescape(encodeURIComponent(skinName));
      ...
      cell.isFood = isFood; cell.isVirus = isVirus;
      cell.agitated = isAgitated; cell.isFriend = isFriend;
      cell.accountID = accountId;
    }
    const eatenCount = reader.readUInt16LE();
    for (let i = 0; i < eatenCount; i++) {
      const cellId = reader.readUInt32LE();
      if (this.playerCells[cellId]) this.playerCells[cellId].destroy(this);
    }
  ...
  checkEnemies(x, y, size) {
    ...
    for (const cell of Object.values(this.playerCells)) {
      ...
      if (isBigger && isClose) enemies.push({ dx, dy, distance, sizeRatio });
    }
    return enemies;
  }
  ...
  nearestPlayer(x, y, size) {
    ...
    if (enemies.length === 0) {
      return { x: this.client.userX / this.rX + this.offsetX, y: this.client.userY / this.rY + this.offsetY };
    }
  ...
  nearestEntity(type, x, y, size) {
    ...
    for (const cell of Object.values(this.playerCells)) {
      let isValid = false;
      switch (type) {
        case "isFood": isValid = !cell.isFriend && !cell.isVirus && cell.isFood && !cell.agitated; break;
        case "isVirus": isValid = !cell.isFriend && cell.isVirus && !cell.isFood && !cell.agitated; break;
      }
      if (!isValid) continue;
      ...
      if (type === "isFood") {
        ...
        for (const enemy of enemies) {
          if (helper.calculateDistance(enemy.dx + x, enemy.dy + y, cell.x, cell.y) < 1000) { isDangerous = true; break; }
        }
        if (isDangerous) continue;
      }
      if (distance < minDistance) { nearest = cell; minDistance = distance; }
    }
  ...
  move() {
    ...
    for (const { x, y, size } of cells) { center.x += x; center.y += y; center.size += size; }
    center.x /= cellCount; center.y /= cellCount;
    const playerTarget = this.nearestPlayer(center.x, center.y, center.size);
    const foodTarget = this.nearestEntity("isFood", center.x, center.y, center.size);
    const virusTarget = this.nearestEntity("isVirus", center.x, center.y, center.size);
    const mouseDistance = helper.calculateDistance(center.x, center.y, this.client.userX / this.rX, this.client.userY / this.rY);
    if (!this.isAlive) return;
    let targetX = playerTarget.x, targetY = playerTarget.y;
    this.isNearMouse = mouseDistance < 4000 + helper.size2mass(center.size) * 0.5;
    if (!this.client.isAlive) { targetX = playerTarget.x; targetY = playerTarget.y; }
    if (this.followMouse) {
      const useAI = this.client.botAi, useVShield = this.client.botVShield;
      if (useAI && useVShield) {
        if (virusTarget.entity) { targetX = virusTarget.entity.x; targetY = virusTarget.entity.y; }
        else if (foodTarget.entity) { targetX = foodTarget.entity.x; targetY = foodTarget.entity.y; }
      } else if (!useAI && useVShield) {
        if (virusTarget.entity && virusTarget.distance < 10000) { targetX = virusTarget.entity.x; targetY = virusTarget.entity.y; }
      } else if (useAI && !useVShield) {
        if (foodTarget.entity) { targetX = foodTarget.entity.x; targetY = foodTarget.entity.y; }
      }
    } else if (foodTarget.entity) {
      targetX = foodTarget.entity.x; targetY = foodTarget.entity.y;
    }
